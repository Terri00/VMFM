...
public:
	...
	static std::vector<glm::vec3> OrderCoplanarClockWiseDEAD(Plane plane, std::vector<glm::vec3> Points) {
		...
		//Bubble sort the value unti all points are clockwise of the former point in the list
		while (true) {
			bool modified = false;

			for (int i = 0; i < Points.size(); i++) {
				int s0 = i;
				int s1 = i == Points.size() - 1 ? 0 : i + 1; //Loop around

				glm::vec3 a = Points[s0]; glm::vec3 b = Points[s1];
				if (CompareClockWiseNess(plane, center, a, b) < 0) {
					Points[s1] = a; Points[s0] = b; //Swap incorrectly ordered points
					modified = true; //Mark this iteration as modified
				}
			}
			if (!modified) break;
		}
