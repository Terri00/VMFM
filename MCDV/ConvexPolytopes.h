...
public:
	...
	Polytope(std::vector<Plane> planes, bool gen_gl_mesh = true)
	{
		...
		for (int i = 0; i < planes.size(); i++) {
			polygons.push_back(BrushPolygon(planes[i]));
		}
		...
		for (int i = 0; i < planes.size(); i++) {
			for (int j = 0; j < planes.size(); j++) {
				for (int k = 0; k < planes.size(); k++) {
					...
					//Add verts
					polygons[i].vertices.push_back(p);
					polygons[j].vertices.push_back(p);
					polygons[k].vertices.push_back(p);
				}
			}
		}

		std::vector<float> generatedMesh;
		std::vector<float> genNgonmesh;

		float x = polygons[0].vertices[0].x;
		float _x = polygons[0].vertices[0].x;
		float y = polygons[0].vertices[0].y;
		float _y = polygons[0].vertices[0].y;
		float z = polygons[0].vertices[0].z;
		float _z = polygons[0].vertices[0].z;
		...
		for (int i = 0; i < polygons.size(); i++) {
			...
			for (int j = 0; j < polygons[i].vertices.size(); j++)
			{
				...
				{
					if (newlist[k] == polygons[i].vertices[j])
					{
						found = true; break;
					}
				}
				...
				newlist.push_back(polygons[i].vertices[j]);

				x = polygons[i].vertices[j].x > x ? polygons[i].vertices[j].x : x;
				_x = polygons[i].vertices[j].x < _x ? polygons[i].vertices[j].x : _x;

				y = polygons[i].vertices[j].y > y ? polygons[i].vertices[j].y : y;
				_y = polygons[i].vertices[j].y < _y ? polygons[i].vertices[j].y : _y;

				z = polygons[i].vertices[j].z > z ? polygons[i].vertices[j].z : z;
				_z = polygons[i].vertices[j].z < _z ? polygons[i].vertices[j].z : _z;
			}

			polygons[i].vertices = newlist;

			//Reorder vertices
			if (polygons[i].vertices.size() < 3)
				continue;

			std::vector<glm::vec3> points = Plane::OrderCoplanarClockWise(polygons[i].plane, polygons[i].vertices);
			polygons[i].vertices = points;
			for (int j = 0; j < points.size() - 2; j++) {
				...
				generatedMesh.push_back(polygons[i].plane.normal.x);
				generatedMesh.push_back(-polygons[i].plane.normal.z);
				generatedMesh.push_back(-polygons[i].plane.normal.y);
		...
		NWU = glm::vec3(-x, z, y);
		SEL = glm::vec3(-_x, _z, _y);

		if (gen_gl_mesh)
		{
			Mesh* m = new Mesh(generatedMesh);
